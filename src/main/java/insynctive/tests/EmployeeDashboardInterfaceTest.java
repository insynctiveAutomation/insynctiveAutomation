...
public class EmployeeDashboardInterfaceTest extends TestMachine {
    ...
    @ParametersFront(
            ...
            labels = {"Birth Date", "Gender", "Marital Status", "SSN", "Primary Phone",
                    "US Address Street", "US Address Second Street", "US Address City",
                    "US Address State", "US Address Zip Copde", "US Address County", "US Address Same As Home"})
    public void updatePersonalInformationWithErrors(@Optional("TestID") Integer testID) throws Exception {
        changeParamObject(testID);
        try {
            EmployeeDashboardPage employeeDashboard = new EmployeeDashboardPage(driver, properties.getEnvironment());
    ...
    @Parameters({"TestID"})
    @ParametersFront(
            attrs = {ParamObjectField.WAIVER_FORM1, ParamObjectField.WAIVER_FORM2, ParamObjectField.WAIVER_FORM3},
            labels = {"Waiver Form1", "Waiver Form2", "Waiver Form3"})
    public void fillAndSignWaiver(@Optional("TestID") Integer testID) throws Exception {
        changeParamObject(testID);
        try {
            EmployeeDashboardPage employeeDashboard = new EmployeeDashboardPage(driver, properties.getEnvironment());
            Set<String> waivers = new HashSet<String>();
            waivers.add(paramObject.getWaiverForm1());
            waivers.add(paramObject.getWaiverForm2());
            waivers.add(paramObject.getWaiverForm3());

            for (String waiverForm : waivers) {
                Debugger.log("Filling => " + waiverForm);
                employeeDashboard.fillAndSignWaiver(waiverForm);
            }

            boolean result = true;

            setResult(result, "Fill and Sign Waivers");
            Debugger.log("fillAndSignWaiver => " + result, isSaucelabs);
            assertTrue(result);

        } catch (Exception ex) {
            failTest("Fill and Sign Waivers", ex, isSaucelabs);
            assertTrue(false);
        }
    }
    ...
    @ParametersFront(
            ...
            labels = {"Medical Benefit Company", "Dental Benefit Company", "Vision Benefit Company"})
    public void fillAndSignBenefit(@Optional("TestID") Integer testID) throws Exception {
        ...
        try {
            ...
            for (String benefitName : benefits) {
                Debugger.log("Filling => " + benefitName);
